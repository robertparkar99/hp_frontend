...
const ViewSkill: React.FC<ViewSkillProps> = ({ skillId, formType, onClose, viewMode = "full" }) => {
  const [sessionUrl, setSessionUrl] = useState<string>("");
  const [sessionToken, setSessionToken] = useState<string>("");
  const [sessionOrgType, setessionOrgType] = useState<string>();
  const [sessionSubInstituteId, setessinSubInstituteId] = useState<string>();
  ...
  const renderKaabWithProficiencyLevels = (type: 'knowledge' | 'ability' | 'attitude' | 'behaviour') => {
    let levels: any[] = [];
    let activeTab: any;
    ...
    if (empty(levels)) {
      return <div className="text-center text-gray-500 text-sm mt-4">No {type} data available</div>;
    }
          ...
          {levels.find((k: any) => k.proficiency_level === activeTab) ? (
            <div>
              <h3 className="text-lg font-semibold text-[#2060E6] mb-4">
                Level {activeTab} - {type.charAt(0).toUpperCase() + type.slice(1)}
              </h3>
                ...
                {levels
                  ...
                  levels
                    ...
                    ?.items.map((item: any, itemIndex: number) => (
                      <li className="bg-[#ebe3f3] p-3 rounded-lg" key={itemIndex}>
                        <div className="flex items-start">
                          <i className="fa fa-chevron-circle-right mr-2 mt-1 text-[#4135ff]" aria-hidden="true"></i>
                          <div>
                            <strong className="text-[#2060E6]">{item.classification_item || item.application || 'N/A'}</strong>
                            {item.classification_category && (
                              <div className="text-sm text-gray-600 ml-2 mt-1">
                                <span className="block"><strong>Category:</strong> {item.classification_category}</span>
                                {item.classification_sub_category && (
                                  <span className="block"><strong>Sub-Category:</strong> {item.classification_sub_category}</span>
                                )}
                              </div>
                            )}
                          </div>
            ...
            <div className="text-gray-400 italic text-center">Select a proficiency level</div>
          )}
        </div>
      </>
    );
