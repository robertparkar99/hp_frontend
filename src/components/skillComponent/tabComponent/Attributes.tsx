...
const AttitudeData: React.FC<Props> = ({ editData }) => {
  ...
  const fetchCategoryData = async () => {
    try {
      const res = await fetch(
        `https://hp.triz.co.in/table_data?table=s_skill_knowledge_ability&filters[sub_institute_id]=3&filters[classification]=attitude&group_by=classification_category`
      );
      const data = await res.json();
      setCategoryOptions(
        Array.from(
  ...
  const fetchInitialData = async () => {
    try {
      // const res = await fetch(
      //   `${sessionData.url}/skill_library/create?type=API&token=${sessionData.token}&sub_institute_id=${sessionData.subInstituteId}&org_type=${sessionData.orgType}&skill_id=${editData?.id}&formType=attitue`
      // );
          const res = await fetch(`https://hp.triz.co.in/skill_library/create?type=API&token=929|fRP90F3cElCvyigHxpTytEJ5GPUr4ZJddh80PTxl205798a3&sub_institute_id=3&org_type=Healthcare&skill_id=2524&formType=attitude`);
      const data = await res.json();
      setSubmittedData(data.userApplicationData || []);
      if (editData && data.userApplicationData?.length) {
        // Pre-fill form for editing if needed
        const first = data.userApplicationData[0];
  ...
  const columns = [
    {
      ...
      selector: (row: AttitudeEntry) => row.proficiency_level ?? "",
      sortable: true,
      wrap: true,
    },
     {
            ...
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="space-x-2">
                ...
                <PdfExportButton
                  ...
                  buttonText={<span className="mdi mdi-file-pdf"></span>}
                />
              </div>
            </div>
