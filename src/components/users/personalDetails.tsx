...
// const PersonalDetails: React.FC<userDetailsprops> = ({ userDetails,userdepartment, userJobroleLists, userLOR, userProfiles, userLists, sessionData }) => {
//   // Form state
...
//       formDataToSend.append('department', formData.personal.department);
...
//                               key={department.id}
//                               value={department.id}
//                               selected={userDetails?.allocated_standards == department.id}
//                             >
...
const PersonalDetails: React.FC<userDetailsprops> = ({ userDetails, userdepartment, userJobroleLists, userLOR, userProfiles, userLists, sessionData }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Only append the image file if it exists (new upload)
      if (formData.personal.imageFile) {
        formDataToSend.append('image', formData.personal.imageFile);
      } 
      else if (formData.personal.image) {
        // If no new file but image URL exists, send the existing image path
        formDataToSend.append('image', formData.personal.image);
      }
                  ...
                  <div className="form-row">
                    <div className="input-field">
                      ...
                      <select
                        className="w-full h-[35px] px-[14px] py-[6px] rounded-[18px] bg-[#eff7ff] text-[#393939] text-[14px] font-normal font-inter border-none outline-none shadow-[inset_0px_2px_8px_rgba(0,0,0,0.2)]"
                        value={formData.personal.user_profile_id}
                        onChange={(e) => handleInputChange('personal', 'user_profile_id', e.target.value)}
                      >
                        <option value="">User Profile</option>
                        ...
                        {userLists && userLists.length > 0 ? (
                          userLists.map((userList: any) => (
                            <option
                              key={userList.id}
                              value={userList.id}
                              selected={userDetails?.employee_id == userList.id}
                            >
                              {userList.first_name} {userList.middle_name} {userList.last_name}
                            </option>
                          ))
                        ) : (
