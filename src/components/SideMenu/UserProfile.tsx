...
export const UserProfile: React.FC<UserProfileProps> = (props) => {
  ...
  const [imgSrc, setImgSrc] = React.useState(defaultImage);

  React.useEffect(() => {
    if (props.UserProfileProp?.userimage && props.UserProfileProp.userimage !== "") {
      setImgSrc(
        `https://s3-triz.fra1.cdn.digitaloceanspaces.com/public/hp_user/${props.UserProfileProp.userimage}`
      );
    } else {
      setImgSrc(defaultImage);
    }
  }, [props.UserProfileProp?.userimage]);
  ...
  return (
    <header
      className="flex gap-6 self-start text-lg leading-none mt-4 p-2"
    ...
    >
      <div className="flex gap-2">
        <img
          ...
          alt="User icon"
          className="object-contain shrink-0 rounded-full w-[40px] h-[40px]"
          onError={() => setImgSrc(defaultImage)} // 👈 ensures fallback always works
        />

        {props.UserProfileProp?.firstName ? (
          <h1 className="my-auto basis-auto text-[16px]">
            {props.UserProfileProp.firstName + " " + props.UserProfileProp.lastName}
          </h1>
        ) : (
          <h1 className="my-auto basis-auto">User Profile</h1>
        )}
      ...
      <img
        ...
        className="object-contain shrink-0 my-auto rounded aspect-square w-[25px]"
        alt="Settings"
        onClick={() => handleLeftPanel("Organizational Development")}
      />
    </header>
  );
};
