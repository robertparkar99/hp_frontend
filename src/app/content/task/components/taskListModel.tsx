...
//     taskListArr.forEach((task: any, index: number) => {
...
//       // Append each task individually as the backend expects
//       payload.forEach((task, index) => {
//         formDataToSend.append(`task_details[${index}][task_title]`, task.task_title);
//         formDataToSend.append(`task_details[${index}][task_description]`, task.task_description);
//         formDataToSend.append(`task_details[${index}][repeat_days]`, task.repeat_days);
//         formDataToSend.append(`task_details[${index}][TASK_DATE]`, task.TASK_DATE);
//         formDataToSend.append(`task_details[${index}][task_allocated]`, task.task_allocated);
//         formDataToSend.append(`task_details[${index}][task_type]`, task.task_type);
//       });

//       // Add common parameters
//       formDataToSend.append("TASK_ALLOCATED_TO", selectedEmployees);
//       formDataToSend.append("formType", "BulkTask");

//       const response = await fetch(
//         `${sessionData.url}/task?type=API&token=${sessionData.token}` +
//         `&sub_institute_id=${sessionData.subInstituteId}` +
//         `&org_type=${sessionData.orgType}&syear=${sessionData.syear}&user_id=${sessionData.userId}`,
//         {
//           method: "POST",
//           headers: {
//             Authorization: `Bearer ${sessionData.token}`,
...
//       const result = await response.json();
//       console.log('Bulk Task Result:', result);
      
//       if (response.ok) {
//         alert("Bulk tasks assigned successfully!");
//         // Reset selection
//         setSelectedRows([]);
//         setSelectAll(false);
//       } else {
//         throw new Error(result.message || "Failed to assign bulk tasks");
//       }
//     } catch (error) {
