...
const TaskManagement = () => {
  ...
  const fetchObserver = async () => {
    try {
      const res = await fetch(
        `${sessionData.url}/table_data?table=tbluser&token=${sessionData.token}&filters['status']=1&filters[sub_institute_id]=${sessionData.subInstituteId}`
      );
      const data = await res.json();
    ...
    {taskList.length > 0 && (
      ...
      <Dialog open={showBulkUpload} onOpenChange={handleBulkUploadDialogChange}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto rounded-xl p-8">
          ...
          <div className="mt-4 rounded-2xl border-2 border-dashed border-blue-400 bg-gradient-to-b from-blue-50/70 to-white px-6 py-10 text-center">
