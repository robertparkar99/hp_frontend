...
const triggerMenuNavigation = (menu: string) => {
  // if you don't want to save a value, save empty string
  ...
  return (
    <div className="w-full max-w-[1560px] mx-auto bg-white relative">
      {/* Header */}
                  ...
                  <div className="md:col-span-2">
                    <div className="bg-white rounded-xl p-6 shadow-md border border-gray-200">
                      {/* <div className="flex justify-between items-center mb-8">
                        ...
                        <button className="flex items-center gap-2 px-8 py-2 rounded-full text-white font-semibold transition duration-300 ease-in-out bg-gradient-to-r from-blue-500 to-blue-700 hover:from-blue-600 hover:to-blue-800 shadow-md disabled:opacity-60">
                      ...
                      <div className="flex justify-between items-center mb-8">
                        {selectedTask.tasks[0]?.type && (
                          // <h2 className="flex-grow text-base font-bold text-gray-800 truncate">
                          <h2 className="flex-grow text-base font-bold text-gray-800 break-words">
                            {selectedTask.tasks[0].type}
                          </h2>
                        )}
                        <button
                          // onClick={triggerMenuNavigation}
                          onClick={() => triggerMenuNavigation("task/taskManagement.jsx")}
                      ...
                      <div className="space-y-6">
                        ...
                        {/* {selectedTask.task_attachment[0]?.type && (
                          <DetailCard
                            title="Attachment"
                            value={selectedTask.task_attachment[0].type}
                          />
                        )} */}
                        {selectedTask.task_attachment[0]?.type && (
                          <DetailCard
...
function EmployeeAvatars({
  ...
  return (
    <div className="flex -space-x-2">
      {employees
        .filter((emp) => emp.image)
        .map((emp, idx) => (
          <img
            key={idx}
            src={emp.image}
            alt={emp.allocated_to}
            title={emp.allocated_to}
            className="w-8 h-8 rounded-full border-2 border-white object-cover"
          />
        ))}
      <span className="text-xs text-gray-700 ml-2" style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{employees[0]?.allocated_to}</span>
    </div>
  );
}
...
//   return (
//     <span
//       className={`min-w-[80px] text-center px-2 py-0.5 text-[10px] sm:text-xs font-medium rounded-full ${status}`}
//     >
//       {status}
//     </span>
//   );
