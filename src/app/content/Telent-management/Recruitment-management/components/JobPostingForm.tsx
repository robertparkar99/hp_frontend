...
// //       try {
// //         // Get the department name from the departments array
...
// //         // Auto-fill skills if available and we're not editing, or if editing and skills are not already set
// //         if (transformedSkills.length > 0) {
// //           if (!editingJob || (editingJob && !formData.skillsRequired)) {
...
// //       fetchJobRoleSkills();
// //     }
// //   }, [formData.title, sessionData, open, jobRoles]);
...
// //       // First, set the basic form data from editingJob
...
// //       // Set selected skills from editing job
...
// //   // Special effect to handle department selection for editing job
// //   useEffect(() => {
// //     if (editingJob && isInitialized && departments.length > 0) {
// //       // Find the department ID that matches the editing job's department
...
// //         console.log("Setting department from editing job:", matchingDepartment.department);
// //         setFormData(prev => ({
// //           ...prev,
...
// //       }
// //     }
// //   }, [editingJob, departments, isInitialized, formData.department]);

// //   // Special effect to handle job title selection for editing job after job roles are loaded
// //   useEffect(() => {
// //     if (editingJob && isInitialized && jobRoles.length > 0 && formData.department) {
// //       // Find the job role ID that matches the editing job's title
...
// //         }));
// //       } else if (!matchingJobRole && editingJob.title) {
// //         console.warn("No matching job role found for:", editingJob.title);
// //       }
// //     }
// //   }, [editingJob, jobRoles, isInitialized, formData.department, formData.title]);
...
// //     const selectedJobRole = jobRoles.find(role => role.id === formData.title);
// //     formDataToSend.append("title", selectedJobRole ? selectedJobRole.jobrole : formData.title);
...
// //       const successMessage = editingJob ? "✅ Job Updated Successfully!" : "✅ Job Posted Successfully!";
...
// //                 <select
// //                   multiple
// //                   value={[]} // Empty array since we're using tags for selection
// //                   onChange={(e) => {
...
// //                         selectedSkills.includes(skill.SkillName) ? 'bg-gray-100 text-gray-400' : ''
// //                       }`}
// //                     >
// //                       {skill.SkillName}
...
//       // Find the job role that matches the editing job's title
//       const matchingJobRole = jobRoles.find(
//         role => role.jobrole === editingJob.title
//       );
...
//         console.warn("No matching job role found for:", editingJob.title);
//         // If no match found, keep the original title as string
//         // This allows the dropdown to show "Select job title" but preserves the value
//       }
//     }
...
//     // Check if formData.title is an ID from jobRoles
//     const selectedRole = jobRoles.find(role => role.id === formData.title);
//     if (selectedRole) {
//       return selectedRole.jobrole;
//     }
    
//     // If not found in jobRoles, it might be the original title string from editingJob
...
//     // Use the selected job role as title - handle both ID and string cases
...
const JobPostingForm = ({ open, onOpenChange, onSave, editingJob }: JobPostingFormProps) => {
  ...
  // Store the original editing job title for comparison
  const [originalJobTitle, setOriginalJobTitle] = useState<string>("");
  ...
  useEffect(() => {
    if (open && editingJob) {
      ...
      // Store the original job title
      setOriginalJobTitle(editingJob.title);
      
      // First, set the basic form data from editingJob
      const initialFormData = {
        title: editingJob.title || "", // Keep as string initially
        department: editingJob.department || "",
        location: editingJob.location || "",
        employmentType: editingJob.employmentType || "",
        experienceRequired: editingJob.experienceRequired || "",
  ...
  useEffect(() => {
    if (editingJob && open && jobRoles.length > 0 && formData.department) {
      console.log("Looking for job role matching original title:", originalJobTitle);
      console.log("Available job roles:", jobRoles);
      
      // Find the job role that matches the original editing job title
      const matchingJobRole = jobRoles.find(
        role => role.jobrole === originalJobTitle
      );
      
      if (matchingJobRole) {
        console.log("Found matching job role:", matchingJobRole.jobrole, "ID:", matchingJobRole.id);
        // Set the ID as the form value for the dropdown
        setFormData(prev => ({
          ...prev,
          title: matchingJobRole.id
        }));
      } else {
        console.warn("No matching job role found for:", originalJobTitle);
        // If no match found, keep the original title as string
        // This will show "Select job title" but we need to handle display differently
      }
    }
  }, [editingJob, jobRoles, open, formData.department, originalJobTitle]);
  ...
  // Get job role name for display - handles both ID-based selection and original string
  const getSelectedJobRoleName = () => {
    if (!formData.title) return "";
    
    // If formData.title is an ID from jobRoles, find and return the jobrole name
    const selectedRole = jobRoles.find(role => role.id === formData.title);
    if (selectedRole) {
      return selectedRole.jobrole;
    }
    
    // If not found, it might be the original string from editingJob
    return originalJobTitle || formData.title;
  };

  // Check if current title value matches any job role
  const isTitleInJobRoles = () => {
    return jobRoles.some(role => role.id === formData.title);
  };
  ...
  const buildFormData = (): FormData => {
    ...
    if (selectedJobRole) {
      jobTitle = selectedJobRole.jobrole;
    } else if (originalJobTitle && !isTitleInJobRoles()) {
      // If editing and title not in current job roles, use original title
...
const successMessage = editingJob
  ? `✅ Job Updated Successfully: ${formData.title}`
  : `✅ Job Posted Successfully: ${formData.title}`;

alert(successMessage); // 🔔 Browser alert instead of toast
      ...
      toast({
        title: "Error",
        description: `Failed to ${editingJob ? 'update' : 'create'} job posting`,
        variant: "destructive",
      });
    } finally {
      setIsSubmitting(false);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              >
                <SelectTrigger className={errors.title ? "border-red-500" : ""}>
                  <SelectValue placeholder={loadingJobRoles ? "Loading job titles..." : "Select job title"}>
                    {getSelectedJobRoleName() || "Select job title"}
                  </SelectValue>
                </SelectTrigger>
                <SelectContent>
                  {loadingJobRoles ? (
              ...
              {editingJob && !isTitleInJobRoles() && (
                <p className="text-sm text-amber-600">
                  Current job title not found in dropdown. Select a new one or keep as is.
                </p>
              )}
            </div>
   {/* Rest of the form remains the same */}
