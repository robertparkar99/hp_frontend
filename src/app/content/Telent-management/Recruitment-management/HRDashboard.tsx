...
//     }   
//     try {
//       const deleteUrl = `${sessionData.APP_URL}/api/job-postings/${jobId}?type=API&token=${sessionData.token}&sub_institute_id=${sessionData.sub_institute_id}`;
...
//       if (!response.ok) {
//         const text = await response.text();
//         throw new Error(`Failed to delete job posting: ${response.status} - ${text}`);
//       }
//       await response.json();
//       fetchJobPostings();
//     } catch (err) {   
...
const handleDeleteJob = async (jobId: number) => {
  ...
  try {
    ...
    console.log("User ID for deleted_by:", sessionData?.user_id);
